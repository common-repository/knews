...
if ($Knews_plugin) {
	...
	if ($id_newsletter != 'preview') {
		...
		$theHtml = $results[0]->html_head . $results[0]->html_bodytag;

		if (defined('KNEWS_ADD_HEADER') && KNEWS_ADD_HEADER==1) {
			$url_home = $Knews_plugin->get_localized_home($results[0]->lang);
			$theHtml .= '<p class="knews_website_logo"><a href="' . $url_home . '">' . get_bloginfo('blogname') . '</a></p>';
		}
		$theHtml .= $results[0]->html_mailing;
	}
