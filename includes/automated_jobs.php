				...
				if ($doit && count($pend_posts) != 0) {
					...
					if ($rightnews) {
						...
						if (!$Knews_plugin->im_pro()) {
							knews_debug('drill checkin: ' . $news[0]->name . "\r\n");
							if (knews_create_news($aj, $pend_posts, $news, $fp, false, 0, true)) {
								knews_debug('now we will construct the newsletter: ' . $news[0]->name . "\r\n");
								$news_id = knews_create_news($aj, $pend_posts, $news, $fp, false, 0);
							} else {
...
function knews_create_news($aj, $pend_posts, $news, $fp, $mobile, $mobile_news_id, $check_before=false) {
	...
	if ($check_before && $total_posts > count($pend_posts)) return false;
	if ($check_before) return true;
