...
if ($Knews_plugin) {
	...
	$knews_styles = 'html, body { width:100% !important; -webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;margin:0;padding:0;}';

	// <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
	if ($do_mobile) $theHtml=str_replace('</head>','<meta name="viewport" content="width=480"></head>',$theHtml);

	if (defined('KNEWS_ADD_HEADER') && KNEWS_ADD_HEADER==1) {
		$knews_styles .= 'p.knews_website_logo { font-family:Arial, Helvetica, sans-serif;background:#000;color:#fff;padding:10px 20px;margin:0;border-bottom:#333 1px solid;font-size:12px;line-height:14px; } p.knews_website_logo a { color:#ccc;text-decoration:none; } p.knews_website_logo a:hover { color:#fff;text-decoration:underline; }';
	}

	$theHtml=str_replace('</head>','<style type="text/css">' . $knews_styles . '</style></head>',$theHtml);
