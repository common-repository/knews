...
function knews_safe_js($text) {
	...
	?>
		...
		// Bounced users
		$query='SELECT COUNT(state) AS bounced FROM ' . KNEWS_USERS . ' WHERE state=4';
		$results=$wpdb->get_results($query);
		$bounced_users=$results[0]->bounced;
		
		$total_users = $not_confirmed_users + $active_users + $blocked_users + $bounced_users;
		...
		?>
			<div class="table_float">
				<table border="0" cellpadding="0" cellspacing="0" width="350">
					...
					<tr>
					<td><img src="<?php echo KNEWS_URL; ?>/images/legend_gray.gif" width="13" height="13" alt="3" /> <?php _e('Bounced users:','knews'); ?></td><td align="right"><?php echo $bounced_users; ?></td><td align="right"><?php echo knews_safe_percent($bounced_users, $total_users); ?>%</td>
					</tr>
					<tr class="alt">
			...
			<script type="text/javascript">
				...
				data.addRows([
					...
					['<?php _e('Bounced users:','knews'); ?>', <?php echo $bounced_users; ?>]
				]);
				...
				var options = { 'title':'', 
								...
								chartArea: { left:80, right:20, top:20, bottom:20 },
